  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#e9e3d5' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        {/* Form thêm tài khoản (Chỉ Cấp 1 mới thấy và thao tác) */}
        {currentCallerLevel === 1 ? (
          <div style={{ background: '#fff', padding: '24px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '32px' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '16px' }}>Thêm Quản trị viên Mới</h2>
            
            {/* autoComplete="off" để trình duyệt không tự điền tài khoản cá nhân */}
          ...
          <div style={{ background: '#fff', padding: '16px 24px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '32px', color: '#666' }}>
            ℹ️ Bạn đang đăng nhập bằng tài khoản <strong>Cấp 2</strong> (Không có quyền thêm hoặc xóa tài khoản).
          </div>
        )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#fafafa', borderBottom: '1px solid #eee', fontSize: '13px', color: '#666' }}>
                  ...
                  <th style={{ padding: '12px 24px' }}>Cấp bậc</th>
                  <th style={{ padding: '12px 24px' }}>Ngày tạo</th>
                ...
                {users?.map((u) => {
                  ...
                  return (
                    <tr key={u.id} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={{ padding: '16px 24px', fontWeight: 600 }}>{u.email}</td>
                      ...
                      <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        {currentCallerLevel === 2 ? (
                          ...
                          <span style={{ color: '#aaa', fontSize: '13px' }} title="Hệ thống cần ít nhất 1 Cấp 1">
                            Cấp 1 duy nhất
                          </span>
                        ) : (
